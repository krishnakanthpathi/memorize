...
def list_memories_endpoint(
    ...
    """List memories with optional category or tag filtering."""
...
def get_memory_detail_endpoint(memory_id: str):
    """Get detailed file and storage status for a specific memory."""
...
def delete_memory_endpoint(memory_id: str):
    """Delete a memory from Markdown disk, SQLite index, and ChromaDB vector store."""
...
def get_memory_versions_endpoint(memory_id: str):
    """Get version snapshot history for a memory."""
...
def revert_memory_endpoint(memory_id: str, req: Optional[RevertRequest] = None):
    """Revert a memory to a specific version snapshot."""
